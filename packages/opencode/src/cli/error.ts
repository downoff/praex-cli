...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, praex does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`praex models\` to list available models`,
      `Or check your config (praex.json) provider/model names`,
    ].join("\n")
  }
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`praex auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
