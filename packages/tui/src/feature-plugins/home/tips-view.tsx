...
const TIPS: Tip[] = [
  ...
  "Create {highlight}praex.json{/highlight} for server settings and {highlight}tui.json{/highlight} for TUI settings",
  "Place TUI settings in {highlight}~/.config/praex/tui.json{/highlight} for global config",
  "Add {highlight}$schema{/highlight} to your config for autocomplete in your editor",
  "Configure {highlight}model{/highlight} in config to set your default model",
  "Override any keybind in {highlight}tui.json{/highlight} via the {highlight}keybinds{/highlight} section",
  "Set any keybind to {highlight}none{/highlight} to disable it completely",
  "Configure local or remote MCP servers in the {highlight}mcp{/highlight} config section",
  "Add {highlight}.md{/highlight} files to {highlight}.praex/commands/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.praex/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.praex/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.praex/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent Praex from reading sensitive files",
  "Use {highlight}praex run{/highlight} for non-interactive scripting",
  "Use {highlight}praex --continue{/highlight} to resume the last session",
  "Use {highlight}praex run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}praex serve{/highlight} for headless API access to Praex",
  "Use {highlight}praex run --attach{/highlight} to connect to a running server",
  "Run {highlight}praex update{/highlight} to update to the latest version",
  "Run {highlight}praex auth list{/highlight} to see all configured providers",
  "Run {highlight}praex agent create{/highlight} for guided agent creation",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.praex/themes/{/highlight} directory",
  ...
  "Run {highlight}praex debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} to add your own provider keys alongside the hosted Praex models",
